            ...
            if (this.$store) {
                ...
                helperTypeArray.forEach(type => {
                    ...
                    if (mapType in options) {
                                ...
                                _mappedResult = mapHelper(
                                    typeof source.value === 'function'
                                        ? source.value.call(this, {
                                            ...
                                            // rootCommit: this.$store.commit.bind(this),
                                            // rootDispatch: this.$store.dispatch.bind(this)
                                        })
                                        : source.value
                                );
                        ...
                        const _ctorOptions = options._Ctor[0].options;
                        let mixins;
                        delete _ctorOptions[mapType];
                        if ((mixins = _ctorOptions.mixins) && mixins.length) {
                            mixins.forEach(mixin => {
                                delete mixin[mapType];
                            });
                        }
                    }
                });
